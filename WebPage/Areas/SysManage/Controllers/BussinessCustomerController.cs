    ...
    public class BussinessCustomerController : BaseController
    {
        ...
        private Common.PageInfo BindList(string Province, string CustomerStyle)
        {
            ...
            var list = result.List.Select(p => new
            {
                ...
                p.CompanyName,
                p.IsValidate,
                CompanyProvince =    this.CodeAreaManage.Get(m => m.ID == p.CompanyProvince)==null?null:this.CodeAreaManage.Get(m => m.ID == p.CompanyProvince).NAME,
                CompanyCity = this.CodeAreaManage.Get(m => m.ID == p.CompanyCity)==null?null :this.CodeAreaManage.Get(m => m.ID == p.CompanyCity).NAME,
                CompanyArea = this.CodeAreaManage.Get(m => m.ID == p.CompanyArea)==null?null:this.CodeAreaManage.Get(m => m.ID == p.CompanyArea).NAME,
                p.CompanyTel,
                p.ChargePersionName,
                p.CreateUser,
